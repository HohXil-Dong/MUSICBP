function runteleBPbmfm(path, BPfile, logFile)
...
if ~isfolder(inputDir)
    musicbp_log(logFile, 'ERROR', 'Missing Input directory: %s', inputDir);
...
musicbp_log(logFile, 'INFO', 'Beamforming output directory: %s', fullfile(inputDir, saveDir));
cd(saveDir);
...
for p = 1:ps
    for q = 1:qs
        ...
        tlib(:, p, q) = sd - mean(sd);
        if max(abs(sd)) > 200
            musicbp_log(logFile, 'WARN', ...
                'Beamforming grid point (%d,%d) has |sd| > 200; continuing', p, q);
            continue
        end
    end
end
...
for tl = parr + begin:step:parr + over % for each time window [tl,th]
    ...
    for p = 1:ps
        for q = 1:qs
            ...
            for k = 1:n
                % y(k,:) are all samples from tl to th for station k,
                % assuming [q,p] is the source point.
                y(k, :) = x0(k, floor((tl + sd(k)) * sr):floor((th + sd(k)) * sr - 1));
            end

            Pm(p, q) = sum(sum(y(:, :), 1) .^ 2);
            % one value for point [q,p] at time tl: summation of n*(win*sr),
            % i.e. all samples in the time window of all the n stations
        end
    end
...
musicbp_log(logFile, 'INFO', 'Beamforming back-projection finished: %s', fullfile(inputDir, saveDir));
